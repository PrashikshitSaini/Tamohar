...
// Hardcoded default shlok to display if CSV loading fails
const DEFAULT_SHLOK = {
  chapter: "2",
  verse: "47",
  sanskrit:
  ...
  useEffect(() => {
    const fetchDailyShlok = async () => {
      try {
        ...
        // Generate a deterministic "random" shlok based on today's date
        const today = new Date();
        const dateString = `${today.getFullYear()}-${today.getMonth()}-${today.getDate()}`;
        const randomIndex = Math.abs(
          dateString.split("").reduce((acc, char) => {
        ...
        if (!todayShlok.sanskrit || !todayShlok.english_meaning) {
          console.error("Missing required fields in parsed shlok:", todayShlok);
          throw new Error("Missing required fields in parsed shlok");
        }

        // Update state with the selected shlok
        setShlok(todayShlok);
